...
*/
 for(i = 0; i <last; i++){
   ...
   MPI_Isend(&message,1,MPI_INT, (rank+1)%npes, 101,MPI_COMM_WORLD, &send_request);
   //sum += rank ;
   MPI_Recv(&message,1,MPI_INT, (rank-1+npes)%npes, 101, MPI_COMM_WORLD,  MPI_STATUS_IGNORE);
